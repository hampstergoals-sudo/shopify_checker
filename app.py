...
async def broadcast_hit(result, hit_type):
    """تنشر البطاقة على القناة الثابتة بس بإيموجي بريميوم"""
    
    if not BROADCAST_CHANNEL_ID:
        return

    card_masked = mask_card(result['card'])
...
🥰 Country {country} {flag}"""
    ...
    # ✨ النشر على القناة الثابتة
    try:
        await broadcast_hit(result, hit_type)
    except Exception as e:
...
📝 Response: {results.get('last_response', 'Waiting...')[:40]}
...
BY 𝗠𝗔𝗘𝗦𝗧𝗥𝗢 (@MA_S_t0)"""
        ...
        if result['status'] in ['Charged', 'Approved']:
            try:
                await broadcast_hit(result, result['status'])
            except Exception as e:
                logger.error(f"broadcast_hit error: {e}")
...
async def check_command(event):
    ...
    try:
        ...
        # ✨ حذف ملف البطاقات بعد القراءة فوراً
    ...
    try:
        ...
        async def worker():
            while not queue.empty() and session_key in active_sessions:
                ...
                res = await check_card_with_retry(card, current_proxies, max_retries=1)
