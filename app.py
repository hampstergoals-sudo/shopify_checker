...
MONGODB_URI = os.environ.get('MONGODB_URI', 'mongodb+srv://admen:CgJIttymPwifi4xL@cluster0.7u8r464.mongodb.net/?appName=Cluster0')
MONGODB_DB = os.environ.get('MONGODB_DB', 'shopify_checker')
...
def init_mongodb():
    ...
    try:
        ...
        results_collection = db[MONGODB_COLLECTION]
        # Create indexes for faster queries
        results_collection.create_index([("timestamp", -1)])
        results_collection.create_index([("site", 1)])
        results_collection.create_index([("status", 1)])
...
def save_result_to_mongodb(data):
...
async def make_graphql_request(
    session, graphql_url, params, headers, json_data, max_retries=1
):
    """Makes a GraphQL request with retries on exceptions."""
    ...
    return None, "Request failed after multiple retries"
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            response, text = await make_graphql_request(
                session, graphql_url, params, headers, submit_json_data, max_retries=1
            )
            ...
            for i in range(4):
                response, final_text = await make_graphql_request(
                    session, graphql_url, params, headers, poll_json_data, max_retries=1
                )
...
def shopify_checker():
    try:
        ...
        try:
            ...
            save_result_to_mongodb(mongodb_record)
        except Exception as db_err:
            print(f"MongoDB save skipped: {db_err}")
        ...
        try:
            ...
            save_result_to_mongodb(error_record)
        except:
            pass
...
def get_results():
    """Get all results from MongoDB with pagination"""
    try:
        if results_collection is None:
            return jsonify({"error": "MongoDB not connected", "status": False}), 500
        ...
        if status_filter is not None:
            query['status'] = status_filter.lower() == 'true'
        if site_filter:
            query['site'] = {'$regex': site_filter, '$options': 'i'}
        
        skip = (page - 1) * limit
        total = results_collection.count_documents(query)
...
def get_stats():
    ...
    try:
        ...
        success_count = results_collection.count_documents({"status": True})
        failed_count = total - success_count
        
        unique_sites = len(results_collection.distinct('site'))
        ...
        gateways = list(results_collection.aggregate(gateway_pipeline))
        ...
        return jsonify({
            ...
            "gateways": [{"gateway": g["_id"], "count": g["count"]} for g in gateways if g["_id"]]
        })
    except Exception as e:
        return jsonify({"error": str(e), "status": False}), 500
...
if __name__ == "__main__":
    ...
    if init_mongodb():
        ...
        print("⚠️  Warning: MongoDB not connected. Results will not be saved.")
