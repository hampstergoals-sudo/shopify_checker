...
# ========== Proxy Test (نسخة كودك الأصلي بالحرف) ==========
async def test_proxy(proxy):
    """
    ✅ نسخة كودك الأصلي بالحرف — shopify.com
    """
    try:
...
async def cancel_command(event):
    await event.reply(premium_emoji("✅ Done."), parse_mode='html')
...
async def add_proxy_command(event):
    ...
    try:
        ...
        for i in range(0, len(new_proxies), batch_size):
            ...
            try:
                await status_msg.edit(premium_emoji(
                    f"⭐ <b>Done</b> ✅{len(alive)} ❌{len(dead)} | <b>Total:</b> {tested}"
                ), parse_mode='html')
            except:
                pass
        ...
        await status_msg.edit(premium_emoji(
            f"⭐ <b>Done</b> ✅{len(alive)} ❌{len(dead)} | <b>Total:</b> {len(new_proxies)}\n\n"
            f"💾 <b>Saved to file:</b> {len(load_proxies())}"
        ), parse_mode='html')
